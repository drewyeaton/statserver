...
class SimpleAverage(object):
  ...
  def __unicode__(self):
    return u'<average Mean:%f, StdDev:%f, Min:%f, Max:%f>' % (self.mean, self.stddev, self.minimum, self.maximum)
  
  
  mean = property(get_mean)
  stddev = property(get_stddev)
...
class SimpleMovingAverage():
  ...
  def __call__(self, n):
    stream = self.stream
    stream.append(float(n))  # appends on the right
  ...
  def __unicode__(self):
    return '<simpleMovingAverage Mean:%f, StdDev:%f, Min:%f, Max:%f>' % (self.mean, self.stddev, self.minimum, self.maximum)
  
  
  maximum = property(lambda self: max(self.stream))
  minimum = property(lambda self: min(self.stream))
  stddev = property(lambda self: sqrt((sum([x * x for x in self.stream]) - (sum(self.stream) * sum(self.stream) / self.period)) / (self.period - 1)))
